...
print(os.listdir("E:/Python/pythonProject_4/video_target_segmentation_segnet/tmp/archive/images/")[-5:])
img = tf.io.read_file("E:/Python/pythonProject_4/video_target_segmentation_segnet/tmp/archive/images/yorkshire_terrier_99.jpg")
img = tf.image.decode_png(img)
img = tf.squeeze(img)
plt.imshow(img)
plt.show()

img1 = tf.io.read_file("E:/Python/pythonProject_4/video_target_segmentation_segnet/tmp/annotations/trimaps/yorkshire_terrier_99.png")
img1 = tf.image.decode_png(img1)
plt.imshow(img1)
plt.show()

#读取所有的图片
images = glob.glob("E:/Python/pythonProject_4/video_target_segmentation_segnet/tmp/archive/images/*.jpg")
# glob.glob('*g') : 匹配所有的符合条件/格式的文件，并将其以list的形式返回
print(len(images))
anno = glob.glob("E:/Python/pythonProject_4/video_target_segmentation_segnet/tmp/annotations/trimaps/*.png")
print(len(anno))
...
train_ds = train_dataset.shuffle(buffer_size=train_count).repeat().batch(batch_size) # batch_size:取样间隔 ; repeat:epoch
...
tf.keras.models.save_model(model,'E:/Python/pythonProject_4/video_target_segmentation_segnet/model.h5') # model 保存